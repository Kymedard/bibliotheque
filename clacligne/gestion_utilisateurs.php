...
if (isset($_GET['supprimer_id'])) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if ($stmt->execute([$id])) {
            ...
            // Si l'utilisateur supprimé est celui connecté, détruire la session et rediriger vers login.php
            if (isset($_SESSION['utilisateurs_id']) && $_SESSION['utilisateurs_id'] == $id) {
                session_unset();
                session_destroy();
